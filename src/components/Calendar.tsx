...
export const Calendar: React.FC<CalendarProps> = ({ tenant, selectedDate, onSelectDate, isAdmin = false }) => {
  ...
  const isAvailable = (date: Date) => {
    // Автоматически блочим все прошедшие дни
    if (isBefore(date, today)) return false;
    return availableDatesStr.includes(format(date, 'yyyy-MM-dd'));
  };
  ...
  return (
    <div className="p-5 bg-[var(--bg-card)] rounded-[2rem] mx-4 border border-[var(--border-main)]">
          ...
          return (
            <div key={i} className="flex justify-center items-center relative">
              ...
              <button
                ...
                className={twMerge(
                  clsx(
                    "flex flex-col items-center justify-center rounded-2xl w-full relative z-10",
              ...
              >
                ...
                <span className={clsx(
                    ...
                    isSelected ? "text-[var(--accent-text)]" : "text-[var(--text-main)]",
      ...
      <div className="pt-3 border-t border-[var(--border-main)] flex justify-center mt-2 relative z-10">
        <button
          ...
          className="flex items-center gap-2 text-[13px] font-bold px-5 py-2.5 rounded-2xl text-[var(--text-secondary)] bg-[var(--bg-card)] hover:bg-[var(--bg-card-hover)] border border-transparent hover:border-[var(--border-hover)] active:scale-95"
        >
          {isExpanded ? (
            <>
              <span>Уменьшить</span>
